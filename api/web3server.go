...
func (svr *web3Handler) call(ctx context.Context, in *gjson.Result) (interface{}, error) {
	...
	// Fork PR #6 cf7ab20d3: callProtocolAddr routes all 4 protocol-address calls
	// (Staking / Rewarding / Poll / RollDPoS) through one helper. Upstream v2.4.1
	// added Poll/RollDPoS support inline; the fork form keeps the logic centralized.
...
func (svr *web3Handler) traceTransaction(ctx context.Context, in *gjson.Result) (interface{}, error) {
	...
	// v1.15.11 trace result extraction: tracerWrapper / evmTracer.Unwrap was removed
	// in batch 3 (R4). TraceTransaction now returns a *tracers.Tracer directly that
	// produces JSON-RPC-shaped results via GetResult().
...
func (svr *web3Handler) traceCall(ctx context.Context, in *gjson.Result) (interface{}, error) {
	...
	// v1.15.11 trace result extraction (see traceTransaction above).
	return tracer.(*tracers.Tracer).GetResult()
}
